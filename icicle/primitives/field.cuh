...
#pragma unroll
  ...
  static constexpr DEVICE_INLINE uint32_t add_sub_u32s_device(const uint32_t *x, const uint32_t *y, uint32_t *r, size_t n = (TLC >> 1))
  {
  ...
  static constexpr DEVICE_INLINE uint32_t add_sub_limbs_device(const ff_storage &xs, const ff_storage &ys, ff_storage &rs)
  {
    ...
    uint32_t *r = rs.limbs;
    return add_sub_u32s_device<SUBTRACT, CARRY_OUT>(x, y, r, TLC);
  }
  ...
  static constexpr DEVICE_INLINE uint32_t add_sub_limbs_device(const ff_wide_storage &xs, const ff_wide_storage &ys, ff_wide_storage &rs)
  {
    ...
    const uint32_t *y = ys.limbs;
    uint32_t *r = rs.limbs;
    return add_sub_u32s_device<SUBTRACT, CARRY_OUT>(x, y, r, 2 * TLC);
  }
...
#pragma unroll
  ...
  static DEVICE_INLINE uint32_t mul_n_plus_extra(uint32_t *acc, const uint32_t *a, uint32_t bi, uint32_t *extra, size_t n = (TLC >> 1))
  {
...
#pragma unroll
  ...
  static DEVICE_INLINE void cmad_n(uint32_t *acc, const uint32_t *a, uint32_t bi, size_t n = TLC, uint32_t optional_carry = 0)
  {
    if (CARRY_IN)
...
#pragma unroll
  ...
  static DEVICE_INLINE uint32_t mad_row(uint32_t *odd, uint32_t *even, const uint32_t *a, uint32_t bi, size_t n = TLC, uint32_t ci = 0, uint32_t di = 0, uint32_t carry_for_high = 0, uint32_t carry_for_low = 0)
  {
  ...
  static DEVICE_INLINE void multiply_and_add_short_raw_device(const uint32_t *a, const uint32_t *b, uint32_t *even, uint32_t *in1, uint32_t *in2)
  {
...
#pragma unroll
  ...
  static DEVICE_INLINE void multiply_raw_device(const ff_storage &as, const ff_storage &bs, ff_wide_storage &rs)
  {
    ...
    uint32_t carry1 = add_sub_u32s_device<true, true>(&a[TLC >> 1], a, diffs);
    uint32_t carry2 = add_sub_u32s_device<true, true>(b, &b[TLC >> 1], &diffs[TLC >> 1]);
    multiply_and_add_short_raw_device(diffs, &diffs[TLC >> 1], middle_part, r, &r[TLC]);
    if (carry1)
      add_sub_u32s_device<true, false>(&middle_part[TLC >> 1], &diffs[TLC >> 1], &middle_part[TLC >> 1]);
    if (carry2)
      add_sub_u32s_device<true, false>(&middle_part[TLC >> 1], diffs, &middle_part[TLC >> 1]);
    add_sub_u32s_device<false, true>(&r[TLC >> 1], middle_part, &r[TLC >> 1], TLC);
